...
def test_aggregated_ts_by_eiaid(temp_history_file: str):
    ...
    query = my_athena.utility.aggregate_ts_by_eiaid(
        eiaid_list=["1121", "1123"],
...
def test_aggregate_unit_counts_by_eiaid(temp_history_file: str):
    ...
    query = my_athena.utility.aggregate_unit_counts_by_eiaid(
        eiaid_list=["1121", "1123"], group_by=["build_existing_model.geometry_building_type_recs"], get_query_only=True
    )
...
def test_aggregate_annual_by_eiaid(temp_history_file: str):
    ...
    query = my_athena.utility.aggregate_annual_by_eiaid(
        enduses=enduses, group_by=["build_existing_model.geometry_building_type_recs"], get_query_only=True
    )
...
def test_get_buildings_by_eiaid(temp_history_file: str):
    ...
    query = my_athena.utility.get_buildings_by_eiaids(eiaids=["1123", "1234"], get_query_only=True)
...
def test_get_filtered_results_csvs_by_eiaid(temp_history_file: str):
    ...
    query = my_athena.utility.get_filtered_results_csv_by_eiaid(["4110", "1167", "3249"], get_query_only=True)
...
def test_get_locations_by_eiaids(temp_history_file: str):
    ...
    query = my_athena.utility.get_locations_by_eiaids(eiaids=["4110", "1167", "3249"], get_query_only=True)
...
def test_get_buildings_by_county(temp_history_file: str):
    ...
    query = my_athena.get_buildings_by_locations(
        location_col="build_existing_model.county", locations=["loc1", "loc2"], get_query_only=True
    )
...
def static_test_utility_inferred_types(temp_history_file):
    ...
    df = my_athena.get_buildings_by_locations(location_col="build_existing_model.county", locations=["loc1", "loc2"])
    df = my_athena.utility.get_locations_by_eiaids(
        eiaids=["4110", "1167", "3249"],
    )
    df = my_athena.utility.get_filtered_results_csv_by_eiaid(
        ["4110", "1167", "3249"],
    )
    df = my_athena.utility.get_buildings_by_eiaids(
        eiaids=["1123", "1234"],
    )
    df = my_athena.utility.aggregate_ts_by_eiaid(
        eiaid_list=["1121", "1123"],
        enduses=["end use: electricity: cooling", "end use: electricity: heating"],
        group_by=["time"],
        query_group_size=1,
    )
    df = my_athena.utility.aggregate_unit_counts_by_eiaid(
        eiaid_list=["1121", "1123"],
        group_by=["build_existing_model.geometry_building_type_recs"],
    )
    df = my_athena.utility.aggregate_annual_by_eiaid(
        enduses=enduses,
        group_by=["build_existing_model.geometry_building_type_recs"],
    )

    df = my_athena.utility.calculate_you_bill(
        rate_map={(1, 0, 1): 0.1, (1, 0, 5): 0.2},
        group_by=["build_existing_model.geometry_building_type_recs"],
    )
    print(df)
